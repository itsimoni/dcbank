/*
  # Fix User Registration to Store Password, First Name, Last Name

  1. Changes
    - Updates handle_new_user trigger to properly initialize all fields
    - Ensures ON CONFLICT clause updates password when provided via API
    - Adds explicit handling for password column

  2. Notes
    - Password is stored as plain text (for admin visibility as requested)
    - The trigger creates initial row, API upsert updates with password
*/
...
BEGIN
  ...
  RAISE LOG 'User inserted into users table';
