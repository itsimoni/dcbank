...
export default function PaymentManager() {
  ...
  const [userPaymentData, setUserPaymentData] = useState<SimplePayment | null>(
    null
  );
  ...
  const getAdminLevelDescription = useMemo(() => {
    ...
    if (
      ...
      return "Full Administrator - Can manage payments for all users";
    }
    if (currentAdmin.is_admin && currentAdmin.is_superiormanager) {
      return "Superior Manager - Can manage payments for assigned managers and their users";
    }
    if (currentAdmin.is_manager) {
      return "Manager - Can manage payments for assigned users only";
    }
  ...
  const fetchPaymentData = useCallback(async (userId: string) => {
    try {
      ...
      if (data) {
        ...
        setEditValues({ payments: "0", on_hold: "0", paid: "0" });
      }
    } catch (error) {
  ...
  const savePaymentData = useCallback(async () => {
    ...
    if (!canManagePayments) {
      setMessage({
        type: "error",
        text: "You don't have permission to manage payments for this user",
      });
      return;
    }
    ...
    try {
      const paymentData = {
        ...
        payments: parseFloat(editValues.payments) || 0,
      ...
      setMessage({
        type: "success",
        text: `Payment data updated successfully for ${
      ...
      await fetchPaymentData(selectedUser.id);
    } catch (error: any) {
      console.error("Error saving payment data:", error);
      setMessage({
        type: "error",
        text: `Error: ${error.message || "Failed to save payment data"}`,
      });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (selectedUser) {
      fetchPaymentData(selectedUser.id);
    } else {
      setUserPaymentData(null);
      setEditValues({ payments: "0", on_hold: "0", paid: "0" });
    }
  }, [selectedUser, fetchPaymentData]);
    ...
    return (
      <Card>
        ...
        <CardContent className="text-center py-8">
          ...
          <p className="text-gray-600 mb-4">
            You need admin or manager permissions to manage payments.
          </p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center">
            <Shield className="w-5 h-5 mr-2" />
            Your Access Level - Payment Management
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center">
            ...
            Simple Payment Management - Three Categories Only
          </CardTitle>
        </CardHeader>
                  ...
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    ...
                    {currentAdmin.is_manager && (
                      <p className="text-xs text-blue-600 mt-1">
                        You can only manage payments for users specifically
                        assigned to you
                      </p>
                    )}
                    {currentAdmin.is_admin &&
                      currentAdmin.is_superiormanager && (
                        <p className="text-xs text-blue-600 mt-1">
                          You can manage payments for managers you assigned and
                          their users
                        </p>
                      )}
                  </div>
                    ...
                    <>
                      <Button
                        onClick={() => {
                          ...
                          if (userPaymentData) {
                            setEditValues({
                              payments: userPaymentData.payments.toString(),
                              on_hold: userPaymentData.on_hold.toString(),
                              paid: userPaymentData.paid.toString(),
                            });
                          }
                        }}
                        variant="outline"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card className="border-yellow-200 bg-yellow-50">
                  ...
                  <CardContent>
                    {editMode ? (
                      <div className="space-y-2">
                        <Label htmlFor="payments">Amount Owed ($)</Label>
                        <Input
                          id="payments"
                          type="number"
                          step="0.01"
                          value={editValues.payments}
                          onChange={(e) =>
                            setEditValues({
                              ...editValues,
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-2xl font-bold text-yellow-700">
                          {formatCurrency(userPaymentData.payments)}
                        </p>
                    ...
                    {editMode ? (
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-2xl font-bold text-blue-700">
                          {formatCurrency(userPaymentData.on_hold)}
                        </p>
                    ...
                    {editMode ? (
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-2xl font-bold text-green-700">
                          {formatCurrency(userPaymentData.paid)}
                        </p>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-lg font-semibold text-gray-900">
                          Total Outstanding:{" "}
                          {formatCurrency(
                            userPaymentData.payments + userPaymentData.on_hold
                          )}
                        </p>
                        <p className="text-sm text-gray-600">
                          Paid to Date: {formatCurrency(userPaymentData.paid)}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-gray-600">Last Updated</p>
                        <p className="text-sm font-medium">
                          {userPaymentData.updated_at
                            ? new Date(
                                userPaymentData.updated_at
                              ).toLocaleDateString()
                            : "Never"}
