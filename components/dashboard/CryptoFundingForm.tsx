import { ArrowLeft, Copy, Clock, AlertCircle, Globe } from "lucide-react";
...
export default function CryptoFundingForm({ onBack }: CryptoFundingFormProps) {
  ...
  const fetchWallets = async () => {
    try {
      ...
      if (!fetchError && data) {
        ...
        const userSpecificWallets = data.filter(
          (w: CryptoWallet) => w.user_id === user?.id
        );
        const defaultWallets = data.filter(
          (w: CryptoWallet) => w.user_id === null
        );

        const walletsToUse =
          userSpecificWallets.length > 0 ? userSpecificWallets : defaultWallets;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (selectedCrypto === "eth" || selectedCrypto === "usdt-erc")
        cryptoType = "ethereum";
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-amber-900 mb-2">
                    {pendingRequests.length === 1
                      ? t.pendingCryptoRequestsAlert.replace('{{count}}', String(pendingRequests.length))
                      : t.pendingCryptoRequestsAlertPlural.replace('{{count}}', String(pendingRequests.length))
                    }
                  </h3>
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
                            {t.cryptoPendingStatus}
                          </span>
                        </div>
                        <p className="text-xs text-gray-600">
                          {t.cryptoSubmittedOnLabel}{" "}
                          {new Date(request.created_at).toLocaleDateString()} {t.cryptoAtLabel}{" "}
                  ...
                  <p className="text-sm text-amber-800 mt-3">
                    {t.previousCryptoRequestsProcessing}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-2xl p-12 text-center">
              ...
              <h2 className="text-3xl font-bold text-gray-900 mb-4">
                {t.transactionPendingTitle}
              </h2>
              <p className="text-lg text-gray-600 mb-8">
                {t.transactionPendingMessage}
              </p>
              <div className="bg-gray-50 rounded-lg p-6 mb-8 text-left">
                <h3 className="font-semibold text-gray-900 mb-3">
                  {t.whatHappensNextCrypto}
                </h3>
                <ul className="space-y-2 text-gray-600">
                  <li className="flex items-start gap-2">
                    <span className="text-[#F26623] mt-1">•</span>
                    <span>{t.cryptoStep1}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-[#F26623] mt-1">•</span>
                    <span>{t.cryptoStep2}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="text-[#F26623] mt-1">•</span>
                    <span>{t.cryptoStep3}</span>
                  </li>
                </ul>
              </div>
              <button
          ...
          <>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <label htmlFor="terms" className="text-sm text-gray-700">
                      {t.termsCheckboxCrypto}{" "}
                      <a href="#" className="text-[#F26623] hover:underline">
                        {t.termsAndConditionsCrypto}
                      </a>
                      .
                    </label>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          blockchainAware: e.target.checked,
                        })
                      }
                      className="mt-1 w-4 h-4 rounded border-gray-300 text-[#F26623] focus:ring-[#F26623]"
                    />
                  ...
                  >
                    {isSubmitting ? t.processingCrypto : t.continueCrypto}
                  </button>
                </form>
              </div>
            ...
            <div className="mt-8 flex items-center gap-6">
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-semibold ${
                    currentStep >= 2 ? "bg-[#F26623]" : "bg-gray-300"
                  }`}
                >
                  2
                </div>
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-semibold ${
                    currentStep >= 3 ? "bg-[#F26623]" : "bg-gray-300"
                  }`}
                >
                  3
                </div>
